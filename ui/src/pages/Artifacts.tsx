...
export const ARTIFACT_GROUP_OPTIONS: { value: ArtifactGroupBy; label: string }[] = [
  ...
  { value: "task", label: "Collection Job" },
  { value: "parent_task", label: "Parent collection job" },
];
...
export function Artifacts() {
  ...
  useEffect(() => {
    if (viewingSelectedStack && selectedGroup) {
      setBreadcrumbs([
        { label: "Evidence Packages", href: "/artifacts" },
        { label: `${selectedGroup.issue.identifier} · ${selectedGroup.title}` },
      ]);
    } else {
      setBreadcrumbs([{ label: "Evidence Packages" }]);
    }
  }, [setBreadcrumbs, viewingSelectedStack, selectedGroup]);

  if (!selectedCompanyId) {
    return <EmptyState icon={Package} message="Select a collection network to view evidence packages." />;
  }
  ...
  const emptyMessage = showGroupCards
    ? searching
      ? "No evidence package stacks match this search."
      : "No evidence package stacks yet."
    : searching
      ? "No evidence packages match this search."
      : viewingSelectedStack
        ? "No evidence packages in this stack match the current filters."
        : kind === "all"
          ? "No evidence packages yet. Evidence attached to collection jobs will appear here."
          : "No evidence packages of this type yet.";
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:max-w-sm">
          ...
          <Input
            ...
            placeholder="Search evidence packages..."
          ...
          {draftQuery.length > 0 ? (
            <button
              ...
              aria-label="Clear evidence package search"
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                ...
                variant="outline"
                size="icon"
                aria-label={`Group evidence packages (currently ${artifactGroupByLabel(groupBy)})`}
                title="Group evidence packages"
          ...
          <div className="flex flex-wrap items-center gap-1.5" role="tablist" aria-label="Filter evidence packages by type">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div ref={loadMoreRef} className="flex min-h-10 items-center justify-center pb-2 text-xs text-muted-foreground">
            {isFetchingNextPage
              ? "Loading more evidence packages..."
              : hasNextPage
                ? null
                : isFetching
